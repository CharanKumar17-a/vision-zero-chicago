...
- **Analytical Grain**: `portfolio_id` × `corridor_id` (1,362 canonical detail rows across 36 scenarios; 6,999 detail rows across 192 serving grid scenarios).
...
- **`fact_corridor_treatment_benefits` to `fact_portfolio_project_selections`**: **1-to-Many** (`corridor_id, treatment_id, uncertainty_scenario`).
  - *Lineage Rule*: Phase 4B candidate panel contains 387 unique candidate rows. Phase 5A project selections contain 1,362 canonical detail rows (6,999 serving rows). Each selection row joins back 100% to exactly 1 candidate row with 0 unmatched rows and 0 join expansion.
