...
# Pinned artifact-gate decision. The key is versioned so a conftest change
# that alters the tables above forces a fresh evaluation; bump it when the
# module/test lists change. (pytest --cache-clear also forces a refresh.)
_GATE_CACHE_KEY = "vision_zero/artifact_gate_v3"


def _artifact_dependent_nodeids(items) -> list[str]:  # noqa: ANN001
    """Node IDs of artifact-dependent tests whose pipeline artifacts are absent NOW.

    Evaluated relative to the current working-tree state; intended to be called
    exactly once per checkout (on the first/primary pytest session).
    """
    missing = []
    for item in items:
        node = str(item.nodeid).replace("\\", "/")
        module_file = Path(node.split("::")[0]).name
        candidates = _MODULE_ARTIFACTS.get(module_file)
        skip_tests = _PIPELINE_TESTS.get(module_file)
        if candidates and skip_tests and item.name in skip_tests:
            if not _any_artifact_exists(candidates):
                missing.append(node)
    return missing


def _skip_reason_for(module_file: str) -> str:
    candidates = _MODULE_ARTIFACTS.get(Path(module_file).name, [])
    return (
        "Requires local pipeline artifacts: "
        + " | ".join(candidates)
        + " (generated by local pipeline runs; absent in this checkout/CI "
        "by repository design — run the pipeline locally to execute this test)"
    )
...
def pytest_collection_modifyitems(config, items) -> None:  # noqa: ANN001
    ...
    The decision is pinned on the FIRST pytest session in a working tree and
    reused by every later session in the same tree. This is deliberate:

    A pytest session itself generates git-ignored pipeline files while running
    (e.g. tests/test_corridor_register.py writes
    data/interim/high_crash_corridor_register.csv). If each session re-evaluated
    the gate against the live file system, a SECOND session in an otherwise
    pristine checkout would see those test-created files, conclude "artifacts
    present", un-gate tests that still lack the rest of their prerequisites
    (corridor geometries, spatial snapshots), and fail. That is exactly what
    broke the GitHub Actions verifier: the workflow's own pytest run (step 6)
    created the register CSV, and verify_project.ps1's second pytest run (step
    7) then executed the 8 corridor-geometry tests on a checkout without their
    other artifacts.

    Pinning on the first session reproduces the pristine-checkout decision for
    every later session: on CI the first session sees no artifacts and skips
    exactly the artifact-dependent tests; on a developer machine the first
    session sees the real pipeline artifacts and runs 100% of the tests, and
    every later session keeps running them. Re-evaluate after generating or
    removing pipeline artifacts with ``pytest --cache-clear`` (or by deleting
    the git-ignored ``.pytest_cache`` directory).
    """
    pinned = config.cache.get(_GATE_CACHE_KEY, None)
    if pinned is None:
        pinned = _artifact_dependent_nodeids(items)
        config.cache.set(_GATE_CACHE_KEY, pinned)

    for item in items:
        node = str(item.nodeid).replace("\\", "/")
        if node in pinned:
            item.add_marker(pytest.mark.skip(reason=_skip_reason_for(node.split("::")[0])))
