...
with sc2:
    st.metric("Estimated capital cost", format_currency_compact(sel_tier["c_cost"]), f"Budget: {sel_tier['budget_str']}")
with sc3:
    st.metric("PV safety benefit (comp)", format_currency_compact(sel_tier["pv_comp"]), f"BCR: {format_bcr_compact(sel_tier['bcr_comp'])}")
with sc4:
    econ_bcr_val = sel_tier["pv_econ"] / sel_tier["c_cost"] if sel_tier["c_cost"] > 0 else 0.0
    st.metric("PV benefit (economic-only)", format_currency_compact(sel_tier["pv_econ"]), f"Econ BCR: {format_bcr_compact(econ_bcr_val)}")
with sc5:
    st.metric(
        "Estimated all-severity crashes avoided / year",
        f"{format_count_compact(sel_tier['crashes_averted'])} / yr",
        f"KSI: {format_ksi_compact(sel_tier['ksi_averted'])} / yr",
        help=f"Denominator: {sel_tier['n_sel']} shortlisted corridors in this scenario. Severity scope: All-severity crashes (K, A, B, C, O) with KSI sub-breakdown.",
    )
...
for _, crow in tier_sel_benefits.iterrows():
    ...
    cost_per_crash_str = format_cost_per_unit(c_cost, c_tot, "crash")
    ...
    tier_roster.append({
        ...
        "Estimated All-Severity Crashes Avoided / Yr": f"{c_tot:.2f} / yr",
